...
/*   Updated: 2024/02/26 17:53:24 by anferre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_convert_to_isometric(int **array3d, t_size *size, t_coor *coor)
{
	...
	beta = 3 * M_PI / 4 + size->b;
	while (i < (*size).rows)
	{
		j = 0;
		while (j < (*size).cols)
		{
			coor[j + i * (*size).cols].dx = cos(beta) * j + sin(beta) * sin(arcsin) * i + sin(beta) * cos(arcsin) * array3d[i][j];
			coor[j + i * (*size).cols].dy = cos(arcsin) * i - sin(arcsin) * array3d[i][j];
			j++;
		}
		i++;
	}
