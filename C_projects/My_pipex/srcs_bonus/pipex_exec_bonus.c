...
/*   Updated: 2024/03/30 17:37:22 by anferre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_pipex_parent(int p_fd[2][2], t_cmd *cmd)
{
	if (dup2(cmd->std_fd[0], STDIN_FILENO) == -1)
		return (ft_c_fd(p_fd[0], p_fd[1], cmd->std_fd), -1);
	if (dup2(cmd->std_fd[1], STDOUT_FILENO) == -1)
		return (ft_c_fd(p_fd[0], p_fd[1], cmd->std_fd), -1);
	close(p_fd[(cmd->i) % 2][1]);
	close(p_fd[(cmd->i) % 2][0]);
	if ((cmd->i) < cmd->nb_cmd - 1)
	{
		if (pipe(p_fd[(cmd->i) % 2]) == -1)
		{
			ft_c_fd(p_fd[((cmd->i) + 1) % 2], NULL, cmd->std_fd);
