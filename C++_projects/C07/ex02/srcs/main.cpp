...
void testIntArray( Array<int> &intArray ) 
{
    ...
    for (unsigned int i = 0; i < intArray.size(); ++i) {
        intArray[i] = rand() % 100;  // Fill with random values
...
{
	Array<int> copyArray = intArray;  // Test copy constructor
    Array<int> assignArray(MAX_VAL);  // Test assignment operator
    ...
    copyArray[0] = 9999;  // Change the first element of the copy
...
void testFloatArray()
{
    ...
    for (unsigned int i = 0; i < floatArray.size(); ++i) {
        floatArray[i] = i * 1.5f;  // Fill with incremental values
